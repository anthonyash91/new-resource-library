-- Category dropdown tier counts (local / regional / statewide) when state+county selected.
-- Run if get_filter_facets was already deployed without counts.
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'counties',
      COALESCE(
        (SELECT jsonb_agg(c.county ORDER BY c.county) FROM county_values c),
        '[]'::jsonb
      ),
    'cities',
